...
export class Viewer {
    ...
    constructor(extension: Extension) {
        ...
        this.server = configuration.get('viewer.pdf.internal.server') as string
    ...
    checkViewer(sourceFile: string, respectOutDir: boolean = true) : string | undefined {
        ...
        const url = `${this.protocol}://${this.server}:${this.extension.server.port}/viewer.html?file=/pdf:${encodeURIComponent(encodeURIComponent(encodeURIComponent(pdfFile)))}`
    ...
    getPDFViewerContent(uri: vscode.Uri) : string {
        ...
        const url = `${this.protocol}://${this.server}:${this.extension.server.port}/viewer.html?incode=1&file=/pdf:${uri.authority ? `\\\\${uri.authority}` : ''}${encodeURIComponent(encodeURIComponent(uri.fsPath))}`
        return `
